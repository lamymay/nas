...
import com.arc.nas.repository.mapper.system.TagMapper;
...
@Component
public class FileTagDAOImpl extends ServiceImpl<TagMapper, FileTag> implements FileTagDAO {

    @Override
    public FileTag saveOne(FileTag record) {
        if (record == null) throw new RuntimeException("保存数据预期非空");
        return this.save(record) ? record : null;
    }

    @Override
    public boolean saveAll(List<FileTag> records) {
        if (records == null || records.isEmpty()) throw new RuntimeException("批量保存数据预期非空");
        return this.saveBatch(records, 100);
    }

    @Override
    public boolean update(FileTag record) {
        if (record == null) throw new RuntimeException("保存数据预期非空");
    ...
    @Override
    public boolean updateAll(List<FileTag> records) {
    ...
    @Override
    public List<FileTag> list(MediaTagRequest query) {
        if (query == null) return Collections.emptyList();
        return this.lambdaQuery().like(StringUtils.isNotBlank(query.getDisplayName()), FileTag::getDisplayName, query.getDisplayName()).select().list();
    }

    @Override
    public FileTag getById(Long id) {
        return this.getBaseMapper().selectById(id);
    }

    @Override
    public IPage<FileTag> listPage(SysFilePageable pageable) {
        return this.lambdaQuery().like(StringUtils.isNotBlank(pageable.getKeyword()), FileTag::getDisplayName, pageable.getKeyword())
