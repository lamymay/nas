...
import com.arc.nas.repository.mapper.system.FileResourceMapper;
...
@Component
public class FileResourceDAOImpl extends ServiceImpl<FileResourceMapper, ResourceConfig> implements FileResourceDAO {

    @Override
    public ResourceConfig saveOne(ResourceConfig record) {
        if (record == null) throw new RuntimeException("保存数据预期非空");
        return this.save(record) ? record : null;
    }

    @Override
    public List<ResourceConfig> saveAll(List<ResourceConfig> records) {
    ...
    @Override
    public boolean update(ResourceConfig record) {
        if (record == null) throw new RuntimeException("保存数据预期非空");
        return this.updateById(record);
    }

    @Override
    public boolean updateAll(List<ResourceConfig> records) {
    ...
    public int deleteAll() {
        int deleted = 0;
        for (ResourceConfig resourceConfig : listAll()) {
            if (resourceConfig != null) {
                int delete = this.getBaseMapper().deleteById(resourceConfig.getId());
    ...
    @Override
    public ResourceConfig getById(Long id) {
        return this.getBaseMapper().selectById(id);
    }


    @Override
    public List<ResourceConfig> list(ResourceConfig query) {
        if (query == null) return Collections.emptyList();
        return this.lambdaQuery()
                .like(StringUtils.isNotBlank(query.getRemark()), ResourceConfig::getRemark, query.getRemark())
                .select().list();
    ...
    public List<ResourceConfig> listAll() {
        List<ResourceConfig> list = this.lambdaQuery()
                .orderByDesc(ResourceConfig::getUpdateTime)
                .select().list();
...
//        queryWrapper.eq(SysFileFolder::getKey, key);
    ...
    @Override
    public IPage<ResourceConfig> listPage(SysFilePageable pageable) {
        return this.lambdaQuery()
