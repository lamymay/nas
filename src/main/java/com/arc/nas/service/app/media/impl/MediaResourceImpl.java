...
public class MediaResourceImpl implements MediaResource {
    ...
    @Override
    public BatchResult addTag(AddTagInner addTagInner) {
        Assert.notNull(addTagInner);
        Assert.notEmpty(addTagInner.getFileIds(), "fileIds not blank");
        Assert.notNull(addTagInner.getTagId(), "tagId not null");

        // 1. 获取已存在的 file-tag 关系，避免重复添加
        List<FileTagRelation> existingRelations = fileTagRelationDAO.listByTagId(addTagInner.getTagId());
        Set<Long> existingFileIds = existingRelations.stream().map(FileTagRelation::getFileId).collect(Collectors.toSet());
        final Long tagId = addTagInner.getTagId();
        // 2. 准备要新增的关系
        List<FileTagRelation> recordsToAdd = addTagInner.getFileIds()
                .stream()
                .filter(fileCode -> !existingFileIds.contains(fileCode)) // 过滤掉已有的
                .map(fileId -> {
        ...
        result.setMessage("success count =" + recordsToAdd.size() + "fail count=" + (addTagInner.getFileIds().size() - recordsToAdd.size()));
        return result;
    }
    ...
    public BatchResult removeTags(RemoveTagBatchRequest request) {
        ...
        int removed = fileTagRelationDAO.deleteByFileIdsTagIds(fileTagRelations);
    ...
    private List<FileTagRelation> builtFileTagRelation(RemoveTagBatchRequest request) {
        ...
        for (RemoveTagRequest item : request.getRemoveTagRequests()) {
            ...
            if (item == null || item.getFileId() == null || item.getTagIds() == null || item.getTagIds().isEmpty())
                continue;

            for (Long tagId : item.getTagIds()) {
                if (tagId != null) {
                    temp.add(new FileTagRelation(item.getFileId(), tagId));
                }
            }
        }
        return temp;
    ...
    private GenerateThumbnailItemResult generateThumbnailsImage(GenerateThumbnailConfig config) {
        ...
        for (SysFile file : fileList) {
            ...
            if (doThumbnails || config.isForce()) {
                ...
                MediaItemDTO byIdOrCode = fileService.getMediaItemDTOByIdOrCode(imageThumbnailSysFile.getCode());
                if (byIdOrCode != null) {
                    //  存在相同的文件了
                    continue;
                }
